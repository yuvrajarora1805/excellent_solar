...
export default function EditOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const prodList = Array.isArray(productsData) ? productsData : (productsData.products || []);
      ...
      if (orderData && orderData.order) {
        ...
        setOrderType(o.order_type || 'RETAIL');
        if (o.customer_id) setSelectedCustomerId(String(o.customer_id));
        setCustomerName(o.customer_name || '');
        setCustomerMobile(o.customer_mobile || '');
        ...
        if (o.serials && Array.isArray(o.serials)) {
          setScannedSerials(o.serials.map((s: any) => ({
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onloadend = () => {
        ...
        setVehiclePhotoBase64(base64String);
        setVehiclePhotoPath(file.name);
  ...
  const handleSubmitOrder = async (dispatchImmediately: boolean) => {
    ...
    try {
      ...
      const res = await fetch(`/api/orders/${id}`, {
        ...
        body: JSON.stringify({
          isDraftEdit: true, // Tell API it's a full draft edit
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">
            Edit Order & Dispatch (Scan Solar Barcodes)
          </h1>
          <p className="text-xs text-slate-500">
            Edit Customer Project or Retail Sale orders with Barcode Serial Scanning & Vehicle Loading Proof
          </p>
        </div>
      </div>
                    ...
                    <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                      ...
                      <td className="p-2 text-right">
                        <input
                          ...
                          className="w-16 p-1 border rounded text-right font-bold text-xs bg-background"
                        />
                      </td>
                      <td className="p-2 text-right">
                        <input
                          ...
                          className="w-24 p-1 border rounded text-right font-bold text-xs bg-background"
                        />
                      </td>
